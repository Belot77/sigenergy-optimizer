...
- A battery below 100% cannot use the implicit cheap-FiT full-battery exception; at exactly 100%, only verified `Automated` plus Maximum Self Consumption Stage 2 may open it. That Stage 2 path remains MSC/PV-only: trusted battery discharge above the 0.1 kW tolerance closes it, `Command Discharging (PV First)` cannot implement it, and a blocked opportunity must not claim `Exporting` or `Full battery`.
...
| `battery_discharge_kw_for_pv_only` | battery sensor discharge, else power-balance fallback | kW | PV-only proof | measured classification and MSC transition | Proven if known | Unknown or above the 0.1 kW tolerance prevents either MSC stage and leaves the implicit full-battery exception closed. Independent explicitly configured, normal-price, and special policies retain their own gates. |
| `pv_only_discharge_ok` | discharge known and <= tolerance | boolean | PV-only proof | measured classification and MSC transition | Safety gate | Must stay conservative. |
...
| Full-battery PV-only Stage 1 | genuine Automated observation, day/live PV, fixed 100% top-off, FiT `>= $0.01/kWh`, known battery flow within tolerance, no conflicting special mode/guard, MSC not genuinely observed | helper mode, PV/load, battery flow, SoC, EMS, FiT | export closed; command Maximum Self Consumption | No measured/estimated/breathe probe and no ramp; wait for a later cycle | Regression proves repeated Stage 1 cycles remain at zero. |
| Full-battery PV-only Stage 2 | Stage 1 opportunity remains valid and current Maximum Self Consumption is genuinely observed | same inputs plus current EMS evidence and grid-export entity maximum | direct downward-quantised `min(EXPORT_LIMIT_HIGH, grid-export entity max)` ceiling with Maximum Self Consumption | Explicitly classified PV-only without a measured-surplus setpoint clamp; may bypass Value Gate/import-cost floor; never uses a Command Discharging mode | Unknown or trusted discharge above 0.1 kW, or any other failed guard, removes this high exception and leaves this implicit path closed. Explicitly configured, normal-price, and special policies qualify independently under their own gates. |
