...
| Feed-in price | `FEEDIN_SENSOR=sensor.amber_feed_in_price` | sensor | dollars/kWh | cents = value * `PRICE_MULTIPLIER` | `_read_state` | export tiers, Value Gate, hard guard | Live input | Automatic full-battery PV-only export requires `>= $0.01/kWh` (1.0c/kWh); lower positive values do not qualify. |
...
| `battery_discharge_kw_for_pv_only` | battery sensor discharge, else power-balance fallback | kW | PV-only proof | measured classification and MSC transition | Proven if known | Unknown or above tolerance prevents either MSC stage; independent battery-backed/mixed policy remains separate. |
...
| Full battery export | full SoC and export opportunity | SoC, FiT, forecasts | export limit/EMS | qualifying MSC path uses a high ceiling; other safeguards remain | `test_export_poor_tomorrow.py` plus focused MSC-ceiling tests. |
...
| Full-battery PV-only Stage 2 | Stage 1 opportunity remains valid and current Maximum Self Consumption is genuinely observed | same inputs plus current EMS evidence and grid-export entity maximum | direct downward-quantised `min(EXPORT_LIMIT_HIGH, grid-export entity max)` ceiling with Maximum Self Consumption | Explicitly classified PV-only without a measured-surplus setpoint clamp; may bypass Value Gate/import-cost floor | Unknown/material discharge or any failed guard removes only this high exception; ordinary battery-backed/mixed policy still applies. |
