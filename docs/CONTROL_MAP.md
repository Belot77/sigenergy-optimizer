...
| `pv_surplus_breathe_probe_active` | full-battery, export-clamped, positive-FiT, no-discharge tiny probe when measured/estimated surplus are both below threshold | boolean | decision | PV-only probe classification | Live small probe | Lets hidden PV breathe without using discharge EMS; must back off if battery discharge appears. |
...
| Full-battery hidden-PV breathe probe | full/top-off battery, export clamped near zero, measured and estimated surplus below threshold, plausible live PV, positive FiT, no battery discharge, automated mode | PV/load, SoC, current export limit, battery flow, EMS | tiny capped export probe; no discharge EMS | Stops being PV-only if battery discharge appears | Tests cover active and blocked paths. |
