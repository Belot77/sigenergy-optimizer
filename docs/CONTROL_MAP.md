...
Full-battery MSC policy supplement: 2026-08-24, branch `fix/msc-full-battery-export-ceiling`, based on release/main commit `64c2059adab2c0b829fbdf0390b7efcd80dcf2fc` (`2.3.36-haos47`).
Current checkpoint supplement: 2026-09-13, committed remediation HEAD `067d52cc5e231d4c3ffd4be2d8c0d058bfbf19b2` plus the uncommitted Solar Surplus PV-margin repair.
...
- Measured PV may classify or clamp an export requested by another policy, but it does not open automatic full-battery PV-only export from zero.
- Estimated and hidden PV are diagnostic-only for the automatic full-battery PV-only policy.
...
- If Maximum Self Consumption is not already genuinely observed, the automatic full-battery PV-only policy first keeps export closed while commanding that mode, then waits for a later qualifying cycle. When genuine `Automated` and Maximum Self Consumption observations are already present together with the 100% top-off, known-safe battery flow, FiT `>= $0.01/kWh`, and daytime/live-PV safety gates, it can open `EXPORT_LIMIT_HIGH` directly. The high value is a ceiling, not a request for battery discharge.
- The ordinary price-tier branch has no SoC-based cheap-FiT grant: SoC alone cannot make `_export_tier_limit` non-zero below `EXPORT_THRESHOLD_LOW` at 99%, 100%, or any other SoC. This removes only the implicit full-battery tier shortcut; independently gated spike/solar-override paths and configured policies such as `ALLOW_LOW_MEDIUM_EXPORT_POSITIVE_FIT` retain their own haos52 gates.
- A battery below 100% cannot use the implicit cheap-FiT full-battery exception; at exactly 100%, only verified `Automated` plus Maximum Self Consumption Stage 2 may open it. That Stage 2 path remains MSC/PV-only: trusted load-serving battery discharge with grid export below the meaningful-export threshold is compatible with the ceiling, while meaningful simultaneous battery discharge and grid export and unknown or untrusted relevant flow remain fail-closed. `Command Discharging (PV First)` cannot implement it, and a blocked opportunity must not claim `Exporting` or `Full battery`.
...
| Grid export limit | `GRID_EXPORT_LIMIT=number.sigen_plant_grid_export_limitation` | number | kW | none | `_read_state`, `_apply`, manual/API | export setpoint and authoritative entity `max` bound | Live | Central live output; its valid `max` attribute bounds the automatic MSC high ceiling. |
...
| Grid import power | `GRID_IMPORT_POWER_SENSOR` | sensor | kW | raw `>100` treated as W and divided by 1000 | `_read_state` | battery-flow fallback, price tracking | Live/diagnostic | Separate import sensor avoids signed-grid ambiguity; missing or non-finite remains unknown. |
| Grid export power | `GRID_EXPORT_POWER_SENSOR` | sensor | kW | raw `>100` treated as W and divided by 1000 | `_read_state` | battery-flow fallback, price tracking | Live/diagnostic | Separate export sensor avoids signed-grid ambiguity; missing or non-finite remains unknown. |
| Battery power | `BATTERY_POWER_SENSOR` | sensor | kW | raw `>100` treated as W and divided by 1000; optional invert | `_read_state` | PV-only proof | Live input | Required to rule out battery discharge. Missing or non-finite means not proven. |
| Battery SoC | `BATTERY_SOC_SENSOR` | sensor | percent | finite numeric input clamped 0-100; invalid defaults to 0 | `_read_state` | reserve, top-off, import/export | Live input | Non-finite input cannot satisfy the 100% top-off gate. |
...
| Feed-in price | `FEEDIN_SENSOR=sensor.amber_feed_in_price` | sensor | dollars/kWh | cents = value * `PRICE_MULTIPLIER` | `_read_state` | export tiers, Value Gate, hard guard | Live input | Automatic full-battery PV-only export requires `>= $0.01/kWh` (1.0c/kWh); lower positive values do not qualify. This minimum is only Stage 2 eligibility and does not create an ordinary tier below `EXPORT_THRESHOLD_LOW`; explicitly configured policies retain their separate gates. |
...
| Demand window | `DEMAND_WINDOW_SENSOR` | binary sensor | boolean | boolean parse | `_read_state` | decision/status | Live input | Blocks automatic import and selects MSC when export is closed or discharge-PV when export is independently open; it does not itself authorise export or alter the normal configured PV MAX. |
...
| Solcast power now | `SOLAR_POWER_NOW_SENSOR` | sensor | kW | raw `>100` treated as W | `_read_state` | `solar_potential_kw` | Estimated live input | Used by planning/general surplus and separate morning-slow logic; never proves or opens the automatic full-battery PV-only ceiling and never authorises HVAC permission. |
...
| Export thresholds | `EXPORT_THRESHOLD_*`, `EXPORT_LIMIT_*`, `ALLOW_*` | dollars/kWh, kW | `_desired_export_limit` | Live | Price-tier export behavior. With `ALLOW_LOW_MEDIUM_EXPORT_POSITIVE_FIT` enabled, FiT `>= $0.01/kWh` may select the separate positive-FiT override even below `EXPORT_THRESHOLD_LOW`; `ALLOW_POSITIVE_FIT_BATTERY_DISCHARGING` governs battery discharge for that policy. This is not implicit full-battery authority. |
...
| Estimated PV initiation | `PV_SURPLUS_ESTIMATED_INIT_ENABLED=true` | boolean | compatibility status only | No live effect for automatic full-battery PV-only export | Retained for configuration compatibility; measured/estimated/breathe discovery no longer opens or ramps this policy. |
...
| Morning Dump | `MORNING_DUMP_ENABLED`, legacy `MORNING_DUMP_HOURS_BEFORE_SUNRISE`, `MORNING_DUMP_MIN_SOC=30` | boolean, hours, percent | morning-dump window/export path | Live | Window duration ends one hour after sunrise; at or below the hard minimum SoC, Morning Dump cannot start or continue regardless of forecast. |
| Morning slow export | minimum FiT plus probe/ramp/margin keys | dollars/kWh, kW | morning slow-charge/export paths | Live | Configured minimum FiT is inclusive; existing separate anti-self-lock path is unchanged by the full-battery MSC ceiling policy. |
...
| Solar surplus bypass | `SOLAR_SURPLUS_BYPASS_ENABLED`, forecast start/stop multipliers, `SOLAR_SURPLUS_MIN_PV_MARGIN`, `SOLAR_SURPLUS_STOP_PV_MARGIN` | boolean/multipliers/kW | `_solar_surplus_bypass` | Live | Entry uses the full forecast and PV margins; only a genuinely previous Solar Surplus-owned high ceiling may use the lower continuation margins. |
...
| `estimated_pv_surplus_kw` | same as `pv_surplus` | kW | trace values | UI/status | Estimated diagnostic | Cannot open automatic full-battery PV-only export. |
...
| `hidden_pv_surplus_kw` | `max(estimated - measured, 0)` | kW | trace values | UI diagnostics | Diagnostic | Cannot open automatic full-battery PV-only export. |
...
| `battery_discharge_kw_for_pv_only` | battery sensor discharge, else power-balance fallback | kW | PV-only proof | raw PV-only checks and ordinary-MSC flow classification | Proven if known | Unknown evidence remains fail-closed. Discharge above 0.1 kW is compatible with the exact-full MSC ceiling only when trusted grid export remains below the meaningful-export threshold; meaningful simultaneous discharge and export closes. |
| `pv_only_discharge_ok` | discharge known and <= tolerance | boolean | PV-only proof | Solar Surplus and other raw PV-only consumers; exact-full diagnostics | Safety gate/diagnostic | Remains conservative for its existing consumers but is not the decisive exact-full MSC transition gate after `067d52c`. |
| `ordinary_msc_flow_ok` | trusted battery-flow and grid-export evidence with no meaningful simultaneous battery discharge and grid export | boolean | optimizer decision | ordinary MSC, Morning Slow, and exact-full MSC transition | Safety gate | Trusted load-serving discharge below the meaningful grid-export threshold is compatible; meaningful simultaneous export and unknown or untrusted relevant flow fail closed. |
| `pv_only_ems_safe` | current EMS is a recognised non-discharge mode (`Maximum Self Consumption` or a supported charging mode) | boolean | PV-only proof | carve-out/initiation | Safety gate | Unknown, unavailable, or discharge EMS state fails closed; the high-ceiling path additionally requires the current EMS to be exactly `Maximum Self Consumption`. |
...
| `sigenergy_mode_observed` / `ems_mode_observed` | current bulk-read entity state is present and not unknown/unavailable | boolean | `_read_state` | MSC transition evidence | Safety evidence | Default, remembered, or substituted values cannot qualify the high ceiling. |
| `pv_only_msc_stage1_active` | genuine Automated observation + all non-EMS MSC opportunity guards + MSC not genuinely observed | boolean | decision | export closed; command MSC | Live transition | Waits for a later observed-MSC cycle and never probes export. |
| `pv_only_msc_high_ceiling_active` | Stage 1 opportunity guards + genuine current Maximum Self Consumption observation | boolean | decision/classifier | high export ceiling, Value Gate/import-cost bypass | Explicit proven control context | This narrow source may be PV-only even when its ceiling exceeds instantaneous measured surplus. |
| `pv_only_msc_high_ceiling_kw` | downward-quantised `min(EXPORT_LIMIT_HIGH, GRID_EXPORT_LIMIT entity max when valid)` | kW | decision | grid export ceiling/status trace | Live ceiling | Maximum Self Consumption determines actual export below the ceiling; ESS discharge power is not the cap for this path. |
| Legacy discovery traces | `pv_surplus_estimated_init_*`, `pv_surplus_breathe_probe_*`, `pv_only_discovery_*`, probe cap/source | mixed | decision trace | compatibility/status | Diagnostic-only | Active/state gates are always false, source is `none`, caps are zero, and they cannot affect live export. |
...
| reason strings | branch and guard explanations | text | decision | UI/helper/status | Diagnostic | Mirror logic; useful to test key safety reasons. A blocked 100%-battery PV-only opportunity must not claim `Exporting` or `Full battery`. |
...
| PV max limiting | standby, negative-price, normal, and other configured PV-cap contexts | PV max config, current limit | PV max power limit | min-change threshold | Ordinary evening/night closed-flow MSC retains normal configured PV MAX; Standby Holdoff keeps its independent intentional cap. UI diagnostics present. |
| Full battery export | full SoC and export opportunity | SoC, FiT, forecasts | export limit/EMS | No SoC value by itself creates an ordinary sub-threshold tier. The implicit cheap-FiT full-battery exception requires fixed-100% verified MSC Stage 2; explicitly configured and other independent policies retain their own gates | `test_export_poor_tomorrow.py` plus focused MSC-ceiling tests. |
| Full-battery PV-only Stage 1 | genuine Automated observation, day/live PV, fixed 100% top-off, FiT `>= $0.01/kWh`, trusted ordinary-MSC battery/grid-export flow, no conflicting special mode/guard, MSC not genuinely observed | helper mode, PV/load, battery flow, grid-export flow, SoC, EMS, FiT | export closed; command Maximum Self Consumption | No measured/estimated/breathe probe and no ramp; wait for a later cycle | Trusted load-serving discharge is compatible; meaningful simultaneous discharge/export and unknown relevant flow remain closed. Regression proves repeated Stage 1 cycles remain at zero. |
| Full-battery PV-only Stage 2 | Stage 1 opportunity remains valid and current Maximum Self Consumption is genuinely observed | same inputs plus current EMS evidence and grid-export entity maximum | direct downward-quantised `min(EXPORT_LIMIT_HIGH, grid-export entity max)` ceiling with Maximum Self Consumption | Explicitly classified PV-only without a measured-surplus setpoint clamp; may bypass Value Gate/import-cost floor; never uses a Command Discharging mode | Trusted load-serving discharge with grid export below the meaningful threshold is compatible. Meaningful simultaneous discharge/export, unknown or untrusted relevant flow, or any other failed guard closes this implicit path; independent policies retain their own gates. |
| Legacy full-battery discovery | old measured/estimated initiation, breathe probe, continuation/ramp inputs or stale trace state | compatibility traces only | none | Active/state fields remain neutral and diagnostic-only | Multi-cycle regression proves no `0 -> probe -> ramp -> veto -> retry` sequence. |
| Anti-curtailment/self-lock avoidance | morning slow-charge probe/ramp conditions | Solcast potential, measured PV/load | separate morning slow-charge export-limit adjustments | Conservative caps | Unchanged by the full-battery MSC ceiling policy; do not combine these paths. |
...
| EMS mode | `_apply`, `_safe_fallback`, `_apply_manual_mode_targets`, `/set_ess` | Stage 2 unconditionally reasserts MSC and waits for exact readback before opening high; before any discharge-mode change, the deliberate export target is written and confirmed first | hard guard/Value Gate can force MSC; failed Stage 2 or export-target settlement falls back closed | High-impact control surface; HA writes are not atomic. |
| Grid export limit | `_apply`, `_safe_fallback`, manual targets, `/set_ess` | final automatic `Decision.export_limit` unless manual/API; write-and-settle-before-discharge ordering also applies when the prior setpoint observation is unavailable | hard guard and Value Gate veto battery-backed/mixed export; zero becomes 0.01 setpoint | Central export safety output. |
...
| `estimated_pv_surplus_kw` | decision trace | potential PV above load | kW | Estimated input/status | Diagnostic-only; not proof and cannot initiate export. |
...
| `pv_surplus_initiation_source`, `pv_surplus_probe_export_cap_kw`, `pv_surplus_estimated_init_*` | decision trace/status | Stage 2 source or neutral legacy-discovery status | text/boolean/kW | Control trace/compatibility | Only `msc_full_battery_high_ceiling` can identify the new opener; legacy active flags stay false and probe caps stay zero. |
| `pv_only_msc_stage1_active`, `pv_only_msc_high_ceiling_*` | decision trace | export-closed MSC transition or qualifying high ceiling | boolean/kW/text | Live control trace | Explicit source for transition, classifier, and guard semantics; do not infer it from a generic high limit. |
...
- PV surplus logic has overlapping names and purposes: `pv_surplus`, `pv_surplus_actual`, `measured_pv_surplus_kw`, `estimated_pv_surplus_kw`, `hidden_pv_surplus_kw`, `solar_surplus_bypass`, neutral legacy-initiation traces, and carve-out flags.
...
- Hidden PV/curtailment: `hidden_pv_surplus_kw`, `hidden_pv_possible`, and PV cap diagnostics; legacy estimated-initiation fields are compatibility-only.
- Export permission: export tier logic, solar surplus bypass, measured PV-only classification/carve-out, explicit MSC Stage 2, Value Gate, hard guard, and forecast safeguards.
...
- Legacy PV-discovery configuration and trace fields, until API/status compatibility is audited separately
...
| Estimated/hidden surplus | diagnostic `estimated_pv_surplus_kw = max(max(pv_kw, solar_power_now_kw) - load_kw, 0)` | automatic full-battery PV-only export permission |
| Hidden PV | `hidden_pv_surplus_kw = max(estimated - measured, 0)` | export permission by itself |
| PV-only export permission | measured `pv_surplus_only_proven` for classifying an already-requested measured-supported limit, or explicit `pv_only_msc_high_ceiling_active` for the direct ceiling | stale discovery state, measured/estimated/breathe initiation, generic `pv_surplus`, or a generic high export limit |
| Automatic MSC high-ceiling cap | downward-quantised `min(EXPORT_LIMIT_HIGH, valid GRID_EXPORT_LIMIT number-entity max when available)` | ESS charge/discharge power or upward rounding |
...
| Final actuator writes | `_apply`, manual target apply, or direct API write for that cycle; automatic high entry verifies MSC and lower-to-discharge transitions settle export first | pre-gate desired trace values |
...
- Unknown/unavailable mode evidence coverage: must not qualify either MSC stage or reactivate legacy discovery.
