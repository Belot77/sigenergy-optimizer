...
| Demand window | `DEMAND_WINDOW_SENSOR` | binary sensor | boolean | boolean parse | `_read_state` | decision/status | Live input | Blocks automatic import and selects MSC when export is closed or discharge-PV when export is independently open; it does not itself authorise export. An active demand window does not by itself activate the generic evening/night battery-only PV cap. |
...
| Morning slow export | probe/ramp/margin keys | kW | morning slow-charge/export paths | Live | Existing separate anti-self-lock path; unchanged by the full-battery MSC ceiling policy. |
...
| PV max limiting | standby, battery-only, normal, and other configured PV-cap contexts | PV max config, current limit | PV max power limit | min-change threshold | Demand windows are excluded from the generic evening/night battery-only trigger; standby holdoff retains its independent intentional cap. UI diagnostics present. |
