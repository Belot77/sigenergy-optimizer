...
6. Package 5 actuator/fallback and chatter work.
   - Actuator/fallback reliability — **complete and automated-validated locally** at `4c9c0e2`; not pushed, deployed, installed, restarted, live-tested, or live-accepted. Application outcomes, fallback results/exceptions, partial failures, remembered applied state, cycle diagnostics, and immediate ordinary-close readback are now handled without treating request success as observed inverter state.
